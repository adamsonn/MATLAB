...
while i<length(LDP)+1
    ...
    if i<(length(LDP)+1)/2
        ...
        adjmatrix(i,(length(LDP)+1))=1-abs(real((1-(SingleBifucprop(optimalflow(i),LDP(i,1)*10^-3,LDP(i,2)*10^-3,LDP(i,3),100)))));
        adjmatrix(i,(length(LDP)+2))=abs(real((1-(SingleBifucprop(optimalflow(i),LDP(i,1)*10^-3,LDP(i,2)*10^-3,LDP(i,3),100)))));
    end
    i=i+1;
...
while i<=length(X)
    ...
    a(i)=length(unique(X(:,i)== 65))-1;
    i=i+1;
end

colatoralmean(n)=mean(a);
