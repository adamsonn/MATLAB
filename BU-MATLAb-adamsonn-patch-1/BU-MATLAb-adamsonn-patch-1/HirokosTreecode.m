...
Parentorg=treeStructure(:,9);
C = setdiff(IDNumber,Parentorg);
...
while pathnumber<length(C)
    ...
    if Pathback(i,pathnumber)==-1
       ...
       Pathback(i+1,pathnumber)=Parentorg(1+Pathback(i,pathnumber));    
       i=i+1;
    end
end
...
flowMatrix(:,2)=flowMatrix(:,2)*10^-3; %Sets initial flow units
% end Ryan Code
...
escapeper1permax=mean(X(numSteps+1,:)-(length(nodes)+1));
...
for i=i:length(nodes)
    flowsearch=[10^-11:1*10^-9:flowMatrix(i,2)];
    probline=(real(SingleBifucprop([10^-11:1*10^-9:flowMatrix(i,2)],nodes(i).Length*10^-2,nodes(i).Diameter*10^-2,nodes(i).gamma,nodes(i).Parent.Diameter)));
    if flowMatrix(i,2)==0
