...
i=1;
while i<476
    adjmatrix(i,476)=SingleBifucprop(.5^max(ceil(log10(abs(LDP(i,4))))),LDP(i,1)*10^-2,LDP(i,2)*10^-2,LDP(i,3),100);
    if i<476/2
        adjmatrix(i,2*i)=(1-SingleBifucprop(.5^max(ceil(log10(abs(LDP(467,4))))),LDP(i,1)*10^-2,LDP(i,2)*10^-2,LDP(i,3),100))/2;
        adjmatrix(i,2*i+1)=(1-SingleBifucprop(.5^max(ceil(log10(abs(LDP(467,4))))),LDP(i,1)*10^-2,LDP(i,2)*10^-2,LDP(i,3),100))/2;
    else
        adjmatrix(i,476)=1;
    end
    i=i+1;
